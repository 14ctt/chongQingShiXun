package com.jero.motelmalltest.common.utils;
...
public class ExcelUtil {
	...
	public static List<Map<String, Object>> readXlsx(MultipartFile excelFile) throws IOException {
		...
		for (int numSheet = 0; numSheet < xssfWorkbook.getNumberOfSheets(); numSheet++) {
			...
			if (xssfRow != null && xssfRow.getLastCellNum() > 0) {// 具有标题行的数据才会做读�?
				cellNum = xssfRow.getLastCellNum();// �?
	...
	public static List<Map<String, Object>> readXls(MultipartFile excelFile) throws IOException {
		...
		for (int numSheet = 0; numSheet < hssfWorkbook.getNumberOfSheets(); numSheet++) {
			...
			if (hssfRow != null && hssfRow.getLastCellNum() > 0) {// 具有标题行的数据才会做读�?
